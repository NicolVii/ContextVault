...
| Smallest compatible change | Add `explainability_snapshot jsonb` (or extend eligibility_snapshot) carrying finalState, drop_reason, WRRF/Final, evidence_requirement, provider plan fields (`chosen_projection_rank`, `required_evidence_disclosure_satisfied`, `required_evidence_projected_to_fit`, `required_evidence_satisfied`, projected packed/budget-dropped IDs, `attainable_utility`, `estimated_total_packed_tokens`, `full_pack_attempt_count`, router version/rank when used), queryDisclosure reason codes (not raw query), user_visible_withhold_notice; widen `channel` check; keep `selected=false` for non-sent rows; store `retrieval_policy_version` on turn or influence |
